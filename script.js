...
// IN / OUT / INTEREST
const inOutInterestDisplay = function (movements) {
  const deposits = movements.reduce((acc, mov) => (mov > 0 ? acc + mov : acc));
  const withdrawals = movements.reduce((acc, mov) =>
    mov < 0 ? acc + mov : acc
  );
  const interest = (deposits * 1.2) / 100;
...
inOutInterestDisplay(movements);
/////////////////////////////////////////////////
